...
%%% used get_required_files_and_products_from_matlab_file.m to copy files
...
figs_to_run = [1:7 11 14:18]; 
    % 0 means figures that didn't make it in the paper
    % 1-7 are the main figures
    % 11-18 are supplemental figures 1-8 (only 1,4-8 are make here)
...
%%%%%%%%%
%%%%%%%%%
%%%%%%%%% General info and directories for rest of scripts. Run first each time.
...
window = [200 200]; %200 window every 200ms
st_nd = [4100 7900]; %1000ms before odor-on and 1000ms after odor-off
...
%Figure 2, 3, 4, S4, S5
if sum(ismember(figs_to_run,[0 2:3 5 14:15]))>0

    disp('Starting single cell responses (calculate_plot_singleneuron_selectivity.m)')
    toplotcells = true;
    %takes ~20 min to generate all the examples in the paper
    
%     lookat = [408 936 184 259 445 491 387 633 335 92 162 697 77 ...
%         1023 676 452 947 247]; %947 was probe example, not using anymore
...
if sum(ismember(figs_to_run,[0 2 4 5 14:16]))>0
    ...
    if saveout
        ...
        toplotcells = false;
        num_avg_over = 50;
        singledat = calculate_plot_singleneuron_selectivity(dirs, neuron_info, ...
            neuronlistnew, mousenames, toplotcells, lookat, labeladd, ...
            odorwindowlabel,sm,num_avg_over);    
    ...
    % [all_Neurons, all_BestSes, all_MeanSes,is_issig_lick,neuron_info_new] = ...
    ...
    if sum(ismember(figs_to_run,[0 2 4 14 15]))>0
        plot_Selectivity_Pie(selectivity_dat,selectivity_params,dirs,figs_to_run)
    end

    %figure 2, 4, s4, s5
    if sum(ismember(figs_to_run,[0 2 4]))>0
        plot_Selectivity_UpDownDuration(singledat,selectivity_dat,selectivity_params,dirs,figs_to_run) 
    end

    %figure 4
    if sum(ismember(figs_to_run,[0 4]))>0
        plot_Selectivity_Comparisons(singledat,selectivity_dat,selectivity_params,dirs,figs_to_run)
        plot_Selectivity_Types(singledat,selectivity_dat,selectivity_params,dirs,figs_to_run)  
    end
    
    %figure S5
    if sum(ismember(figs_to_run,[0 15]))>0
        plot_Selectivity_OdorOff(singledat,selectivity_dat,selectivity_params,dirs,figs_to_run)
    end

    %figure 5, S6    
    if sum(ismember(figs_to_run,[0 5 16]))>0
        plot_Selectivity_proportions(singledat,selectivity_dat,selectivity_params,dirs,figs_to_run)
    end

%     clear singledat
    
    % not used, proportion of neurons modulated for each probe or NTrepeat type    
    if sum(ismember(figs_to_run,0))>0
        plot_Selectivity_AcrossOdors(selectivity_dat,selectivity_params,dirs) 
    end
...
%%%%%%%%%
%%%%%%%%%
%%%%%%%%% Correlation of population activity across stimulus type - figures 6-7
% PV corr, decoding of target, probes and repeats

%figures 6 and 7
if sum(ismember(figs_to_run,[0 6 7]))>0
    
    disp('Starting population activity (calculate_plot_PVcorr.m)')

    %%%% Exclusions and paramters
    
    numNcutoff = 4; %has to be at least this many neurons in that session    
     % (excludes sessions with fewer than 4 neurons in it)
    postdattimes = [100 600]; %post-odor-on window
    excludeLickNeurons = false;  %If exclude lick neurons
    ExclLickTime = false; %true; %If excluding lick trials any trials with a lick
    % that comes before postdatimes(2) (e.g. 600ms post odor onset) is excluded        
    plotPVsessions = false; %whether to run individual sessions    
    if sum(ismember(figs_to_run,[0 7]))>0; plotPVsessions = true; end
    runshuffle = false; %whether to run the shuffle    
    if sum(ismember(figs_to_run,[0 6]))>0; runshuffle = true; end
    PVCorr_lookback = 10; % how many trials to look-back to interpolate 
...
% controls for lick neurons and trials

if sum(ismember(figs_to_run,0))>0
    for ExclLickTime = 0:1
        for  excludeLickNeurons = 0:2
            if (ExclLickTime== 1 && excludeLickNeurons>0) || (excludeLickNeurons==0 && ExclLickTime==0)
                continue
            end
            disp(['Running ExclLickTime ' num2str(ExclLickTime) ', excludeLickNeurons ' num2str(excludeLickNeurons)])

            calculate_plot_PVcorr(dirs, neuron_info, neuronlistnew, ...
                excludeLickNeurons, numNcutoff, postdattimes, ExclLickTime, ...
                selectivity_dat.is_issig_lick,PVCorr_lookback,runshuffle,...
                plotPVsessions,figs_to_run)
        end
    end
end
