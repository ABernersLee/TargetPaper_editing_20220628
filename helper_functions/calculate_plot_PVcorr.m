...
all_labels.numfolds = 10; %10; %size(ns,1); %10-fold decoding, downsmapling to the fewest
% of the repeats/probes

%%%% if downsampling
all_labels.numfolds2 = 20; %15; %1; %20 % numfolds; % iterations of randomly doing
...
% addon = [addon '_zscoredDecoding_zscoredall'];
all_labels.addon = [all_labels.addon '_PVCorrLookback_' num2str(PVCorr_lookback)];

%%%% if downsample
if all_labels.todownsample
    all_labels.addon = [all_labels.addon '_DownsampleTrials' num2str(minhmin)];
end
...
for itype =  2:-1:1 
    ...
    if sum(ismember(figs_to_run,[0 6]))==0 && itype==2
        continue
    end
    ...
    for isession = 1:size(sessionstouse,1)
        ...
        for ineuron = 1:length(neuronlist)
            ...
            if ineuron==1
                ...
                trialdatsession = trialdat; % cat(3,trialdatsession,trialdat);
            else
            ...
            if sum([predat;postdat])>5
               ...
               neurondatsession =  cat(3, neurondatsession,[predat postdat]);
            end
...
%         ns_save=squeeze(neurondatsession(:,2,:)-neurondatsession(:,1,:)); 
%         ns_save=ns;
        ...
        ns_save = ns;
        ns = zscore(ns,1); %scale it for decoding %ABL checking how this 
        % does 6/3/2021
        ...
        [corr_repeat_trial_session0,across_lookback0] = ...
       ...
       %plot
        if all_labels.plotPVsessions && itype==1 && (sum(ismember(figs_to_run,0))>0 ...
                || (sum(ismember(figs_to_run,7))>0 && ...
                strcmp('T',all_labels.mousenames{sessionstouse(isession,1)}) ...
        ...
        t = toc;
        disp(['Done in ' num2str(t/60) ' min, session ' num2str(isession) ' ' all_labels.typelab{itype}])
        
        %take the average over the permuations
        decoded_fold_group = mean(mean(decoded_fold_group,3,'omitnan'),4,'omitnan');
        decoded_fold =  mean(mean(decoded_fold,3,'omitnan'),4,'omitnan'); 
        decoded_fold_cat = mean(mean(decoded_fold_cat,3,'omitnan'),4,'omitnan');           
        decoded_fold_S =  mean(mean(decoded_fold_S,3,'omitnan'),4,'omitnan');
        decoded_fold_cat_S = mean(mean(decoded_fold_cat_S,3,'omitnan'),4,'omitnan');      
        decoded_fold_Sprob =  mean(mean(decoded_fold_Sprob,3,'omitnan'),4,'omitnan');
        decoded_fold_cat_Sprob = mean(mean(decoded_fold_cat_Sprob,3,'omitnan'),4,'omitnan');
...
%repeat type, decoding correct, posterior probability 
% (that it is the right one), trial number
       decode_fold_label = cat(1,decode_fold_label,[decoded_fold (1:size(decoded_fold,1))' ... 
                    ones(size(decoded_fold,1),1)*sessionstouse(isession,2) ...%session
                    ones(size(decoded_fold,1),1)*sessionstouse(isession,1)  trialdatsession(:,2) thelicktime]); %mouse, correct, licktime
       decode_fold_label_cat = cat(1,decode_fold_label_cat,[decoded_fold_cat (1:size(decoded_fold,1))' ... %repeat type, decoding correct, posterior probability that it is NT [I make it P(correct) later], trial number
                    ones(size(decoded_fold,1),1)*sessionstouse(isession,2) ... %session
                    ones(size(decoded_fold,1),1)*sessionstouse(isession,1)  trialdatsession(:,2) thelicktime]); %mouse, correct, licktime
       decoded_fold_group_all = cat(1,decoded_fold_group_all,decoded_fold_group);
       decode_fold_shuffle = cat(1,decode_fold_shuffle,decoded_fold_S);
       decode_fold_cat_shuffle = cat(1,decode_fold_cat_shuffle,decoded_fold_cat_S);
       
       decode_fold_shuffle_prob = cat(1,decode_fold_shuffle_prob,decoded_fold_Sprob);
       decode_fold_cat_shuffle_prob = cat(1,decode_fold_cat_shuffle_prob,decoded_fold_cat_Sprob);
    ...
    %%%%%%%% Labels for plots
    all_labels.ittlabs = {'self';'target';'nontarget';all_labels.typelab{itype}};         
    all_labels.lablabs = {'Target';'NonTarget';[all_labels.typelab{itype} ' 1'];...
        [all_labels.typelab{itype} ' 2'];[all_labels.typelab{itype} ' 3']};        
    ...
    if all_labels.runshuffle
        ...
        %figure 6 figures
        if (sum(ismember(figs_to_run,[0 6]))>0 && itype==2) || (sum(ismember(figs_to_run,0))>0)
            ...
            dirs,all_labels,decode_fold_shuffle_prob,decode_fold_cat_shuffle_prob,...
            itype, figs_to_run)
        ...
        if sum(ismember(figs_to_run,0))>0
            %Across Trials
            plot_Decoding_Across(1,decode_fold_label,decode_fold_label_cat,all_labels,dirs)
    
            % sepecifically for the probes, asking whether probes are more likely
            % to get decoded as NTs over sessions
            if itype==1
                plot_Decoding_Probe2Probe(decode_fold_label,decode_fold_label_cat,all_labels,dirs,decoded_fold_group_all,itype)
            end  
        end
    end
    ...
    if sum(ismember(figs_to_run,[0 7]))>0
        plot_TNT_and_Latency(corr_repeat_trial_session,all_labels,dirs,itype,across_lookback,figs_to_run)
    end 
        
    % old figures I used to use
    if sum(ismember(figs_to_run,0))>0
        % Visualize PV plots changing    
        plot_PV_Imagesc(corr_repeat_trial_session,all_labels,dirs,itype)
          
        % plot the PV comparisons a bunch of different ways
        plot_PV_Across(corr_repeat_trial_session,all_labels,dirs,itype,smd)
    
        % Some PV anovas
        plot_PV_ANOVAS(corr_repeat_trial_session,all_labels,dirs,itype)
    end
