...
decoded_fold_S =  ...
    NaN(size(ns,1),all_labels.numshuff,all_labels.numfolds2,length(thesetrials)); 
...
% if size(ns22,2)<6
%     disp(['Skipping, only ' num2str(size(ns22,2)) ' neurons'])
%     continue
% end

% %%%%%% to exclude when downsample a lot
% A = arrayfun(@(x) min(length(unique(ns22(:,x)))),1:size(ns22,2));      
% ns22(:,A<6) = []; %(size(ns22,1)/numfolds)) = [];  
%need to have at leat 5 different types of trials
% 
%         if size(ns22,2)<3         
%             disp(['LATE too few, ns = ' num2str(length(neuronlist)) ...
%         ', session ' num2str(isession) ' ' typelab{itype}])
%         %                     endearly = true;
%             continue
%         end

        ns2 = zscore(ns22,1); %zscore across those trials                    
...
%                if sum(A<numexlund)>0
%                    disp(['Session ' num2str(isession) ...
%                        ': Excluded ' num2str(sum(A<numexlund))...
%                        ' of ' num2str(size(ns2,2)) ', '...
%                        num2str(size(ns2,2)-sum(A<numexlund))...
%                        ' left'])
%                end
        ...
        %can decode better than chance
         if all_labels.runshuffle  %&& isgoodstill(ifoldrun)
            ...
            for ifold = 1:all_labels.numfolds                   
               ...
               if sum(numfoldschunks~=ifold)==1
                   error('need to write the code for probability here')

% [R1_c,~,postS] = arrayfun(@(x) ...
%    classify(X,Y,catgroupS(numfoldschunks~=ifold,x)),1:numshuff);               
% [R1_cat,~,postSc] = arrayfun(@(x) ...
%    classify(X,Y,catgroup_catS(numfoldschunks~=ifold,x)),1:numshuff);
% decoded_fold_S(numfoldschunks==ifold,:) = ...
%     repmat(catgroup2(numfoldschunks==ifold,1),[1 numshuff])==R1_c; %               
% decoded_fold_cat_S(numfoldschunks==ifold,:) = ...
%     repmat(catgroup_cat2(numfoldschunks==ifold,1),[1 numshuff])==R1_cat;          
               ...
               elseif sum(numfoldschunks~=ifold)>1    
                   ...
                   %%% add the accuracy
                   R1_c2 = cellfun(@(x) x==catgroup2(numfoldschunks==ifold,1),R1_c,'UniformOutput',false);  
                   decoded_fold_S(ind2(numfoldschunks==ifold),:,ifoldrun,itrialtype) = cell2mat(R1_c2); %                           
                   R1_cat2 = cellfun(@(x) x==catgroup_cat2(numfoldschunks==ifold,1),R1_cat,'UniformOutput',false);  
                   decoded_fold_cat_S(ind2(numfoldschunks==ifold),:,ifoldrun,itrialtype) = cell2mat(R1_cat2);     
                   
                   %%% add the probability correct
                   decoded_fold_Sprob(ind2(numfoldschunks==ifold),:,ifoldrun,itrialtype) = post2; %
                   decoded_fold_cat_Sprob(ind2(numfoldschunks==ifold),:,ifoldrun,itrialtype) = squeeze(Sc(:,1,:));       
               end
            end   
         end                 
    end
