function helper_plot_behavior_sessions(sessionsall,mice,mousenames,dirs,figs_to_run)
...
if sum(ismember(figs_to_run,[0 7]))>0
    for itype = 1:2
        ...
        for imouse = 1:length(mice)
            sz = sum(sessionsall(:,1)==mice(imouse) & sessionsall(:,8)==itype);
        ...
        helper_saveandclosefig([dirs.figdir ...
            '\Behavior\NeuralDataSessions\Accuracy\AcrossAllSessionsUsing_ProbeRepeat_' typelab{itype}])
    end
end
...
if sum(ismember(figs_to_run,[0 5]))>0
    ...
    itype = 2;
    A = [sessionsall(sessionsall(:,8)==itype,3);sessionsall(sessionsall(:,8)==itype,4);mean(sessionsall(sessionsall(:,8)==itype,5:7),2,'omitnan')];
    C1 = ones(sum(sessionsall(:,8)==itype),1);
    C = cat(1,C1,C1*2,C1*3);
%     C = reshape(ones(sum(sessionsall(:,8)==itype),1)*(1:3),...
%         [sum(sessionsall(:,8)==itype)*3 1]);
    ...
    for itrialtype = 1:3
        ...
        for imouse = 1:length(mice)
            ...
            mdat(sessionsall(ind,2),imouse) = dat; %dat instead of sessionsall(ind,2) for excluding nonrecording days
    ...
    N = length(unique(sessionsall(sessionsall(:,8)==itype,2)+(sessionsall(sessionsall(:,8)==itype,1)*100)));
    axis tight
    xl = get(gca,'xlim');
    xlim([xl(1)-1 xl(2)+1])
    ylim([-.1 1.1])
    ...
    title({['N = ' num2str(N) ', '  typelab{itype}];[trp ' ANOVAp = ' num2str(ps(2))]})
    helper_saveandclosefig([dirs.figdir ...
        '\Behavior\NeuralDataSessions\Accuracy\OverSessions_' typelab{itype}])
    writecell(tbl,[dirs.figdir ...
        '\Behavior\NeuralDataSessions\Accuracy\OverSessions_' typelab{itype} '.xlsx'])
end