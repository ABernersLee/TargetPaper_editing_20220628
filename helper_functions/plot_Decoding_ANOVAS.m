...
% using only 1 figure and some anova stats from this (figure 6)
...
if sum(ismember(figs_to_run,0))>0
    for itt = 1:2
        [~,tbl] = anovan(decode_fold_label(decode_fold_label(:,1)==itt,3),...
...
%     [~,tbl,stats] = ...
%         anovan(decode_fold_label_cat(decode_fold_label_cat(:,1)>2,3),...
%         decode_fold_label_cat(decode_fold_label_cat(:,1)>2,[1 4:7]),...
%         'varnames',{'Repeat','Trial','Session','Mouse','Rewarded'},...
%         'continuous',[2,3],'display','off');
    [~,tbl,stats] = ...
