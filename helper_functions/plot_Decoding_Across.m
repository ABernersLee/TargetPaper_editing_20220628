function plot_Decoding_Across(iplottype,decode_fold_label,decode_fold_label_cat,all_labels,dirs,itype, figs_to_run)

%two figures from here are used for figure 6 
...
for icat = 1:2
  for iraw = 1:2
      for icor = 1:3
          ...
          if (sum(ismember(figs_to_run,[0 6]))>0 && icat==2 && iraw==2 && icor==2) || (sum(ismember(figs_to_run,0))>0)
             ...
             for itt = 1:3
                ...
                elseif iraw==2
                       ...
                       [~,~,stats] = anovan(dat(:,3),dat(:,[1 plotind2 6:7]),'varnames',{'Repeat',plotlabeloth,'Mouse','Rewarded'},'continuous',2,'display','off');    
                ...
                if size(datuse,1)==1
                 ...
                 writecell(tbl,[thisdir all_labels.typelab{itype}...
                     ...
                     '_ANOVA_' all_labels.ittlabs{itt+1} '.txt'],'Delimiter','tab')  
    ...
    %                             violinplot1(datuse,dat(:,plotind),'EdgeColor',[.5 .5 .5],'BoxColor',[.5 .5 .5],'ViolinColor',[.5 .5 .5],'MedianColor',[.3 .3 .3]);
                        violinplot1(datuse(~isnan(datuse)),dat(~isnan(datuse),plotind),'EdgeColor',[0 0 0],'BoxColor',[0 0 0],'ViolinColor',[1 1 1],'ViolinAlpha',0,'MedianColor',[0 0 0]);
                    end
    %                     end
    ...
    %              set(gca,'xtick',5:5:25)
                 ...
                 p = (1+sum(abs(r_shuff)>=abs(r)))./(1+all_labels.numshuff);   
             ...
             figure(aa)
             suptitle({[all_labels.typelab{itype} ' Decoding ' catlabel];['Over ' plotlabel 's, ' rawlab ', ' corlab]})
             set(gcf,'Position',[ 2206         173         542         695])
             helper_saveandclosefig([thisdir all_labels.typelab{itype} '_' catlabel '_' rawlab '_' corlab all_labels.addon])
             
            figure(bb)
            for itt = 1:3
                bb.Children(itt).XLim = [min(xl_all(:,1)) max(xl_all(:,2))];
            end
    
             suptitle({[catlabel ' Decoding'];[ rawlab ', ' corlab]})      
             set(gcf,'Position',[ 2206         173         542         695])
             helper_saveandclosefig([thisdir all_labels.typelab{itype} '_' catlabel '_' rawlab '_' corlab all_labels.addon '_Slope'])
          end
