function plot_accuracy_oversessions(dirs,neuron_info,neuronlistnew,figs_to_run)

%%% behavior from sessions/mice with neural recordings

% add something for figure 5 where I have accuracy and latencies during nt
% repeat overtraining
...
if sum(ismember(figs_to_run,[0 5 17 18]))>0
    helper_plot_lick(lickall,mice,mousenames,dirs,figs_to_run)
end

if sum(ismember(figs_to_run,[0 5 7]))>0
    helper_plot_behavior_sessions(sessionsall,mice,mousenames,dirs,figs_to_run)
end
