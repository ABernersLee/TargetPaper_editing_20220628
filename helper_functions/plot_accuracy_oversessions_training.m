...
%         end
        ...
        % alltrial = NaN(1000,30,size(mousenames,2),5);
        ...
        for imouse = 1:size(mousenames,2)
            ...
            if exlnan==2
                ...
                %only can generate averages with the nans, dont know the raw
                %lick times to work out the accuracy, because they werent saved
                %out
                %shaping data - need to differentiate
                ...
                for iday = 1:3
                    if isfile([dirs.dathome '\April\Behavior_training_only\training_mouse_'...
                        ...
                        % the real percents)
            %                 'percent_NONtarget_trials_correct',...
            %                 'percent_target_trials_correct')
            %             alldat(iday,imouse,1) = percent_target_trials_correct;
            %             alldat(iday,imouse,2) = 1-percent_NONtarget_trials_correct;
            
                         disp(['mouse ' num2str(imouse) ' phase_of_training ' num2str(phase_of_training)])
...
%                         if max(times_of_correct_licks_to_port1_across_trials)>2500 || ...
%                                 max(times_of_correct_licks_to_port2_across_trials)>2500
%                             disp('correct lick too late')
%                             disp('*')
%                         end

                        alldat(iday,imouse,1) = T;
                        alldat(iday,imouse,2) = 1-NT;
                        trainingdays = cat(1,trainingdays,[iday phase_of_training]);        
                    end
                end
            ...
            for itrialtype = 1:size(dat,2)
                ...
                elseif exlnan==2
%                     choiceinx2 = arrayfun(@(x) ...
%                         mean(dat(itrialtype).performance(x).within_session_choice_index_every_trial_no_moving_avg==1)...
%                         ,1:length(dat(itrialtype).performance));
    ...
    %                 'avg_session_choice_index');
                ...
                elseif exlnan==2
%                     choiceinx2 = arrayfun(@(x) ...
%                         mean(dat2(itrialtype).performance(x).within_session_choice_index_every_trial_no_moving_avg==1)...
%                         ,1:length(dat2(itrialtype).performance));
        ...
        % %change to this is it is each actual day instead
        %     datind(OTH2-min(OTH2)+1,imouse,1:3,1) = alldatc(OTH2,imouse,1:3);
        %     datind(NTR-min(NTR)+1,imouse,1:3,2) = alldatc(NTR,imouse,1:3);
        %     datind(P-min(P)+1,imouse,1:3,3) = alldatc(P,imouse,1:3);
        %     datind_lick(OTH2-min(OTH2)+1,imouse,1:3,:,1) = alllickc(OTH2,imouse,1:3,:);
        %     datind_lick(NTR-min(NTR)+1,imouse,1:3,:,2) = alllickc(NTR,imouse,1:3,:);
        %     datind_lick(P-min(P)+1,imouse,1:3,:,3) = alllickc(P,imouse,1:3,:);
        ...
        if sum(ismember(figs_to_run,[0 1]))>0
            ...
            A = cat(3,datind(:,:,1,1:3),1-datind(:,:,2,1:3)); %,1-datind(:,:,3,1:3));
            ...
            for istage =  1:3
                A1 = A(E==istage); B1 = B(E==istage); C1 = C(E==istage); D1 = D(E==istage); 
        ...
        if sum(ismember(figs_to_run,[0 7]))>0
            ...
            for itrialtype = 1:size(alldat_AllProbes,3)
                  toplotind = sum(~isnan(alldat_AllProbes(:,1:4,itrialtype)),2)>1;
                  errorbar(mean(alldat_AllProbes(toplotind,1:4,itrialtype),2,'omitnan')...
                      ,std(alldat_AllProbes(toplotind,1:4,itrialtype),[],2,'omitnan')...
                      ./sqrt(sum(~isnan(alldat(toplotind,1:4,itrialtype)),2)),...
                      '-','LineWidth',2)
            end
            ylim([-.1 1.1])
            xl = get(gca,'xlim');
            ...
            helper_saveandclosefig([dirs.figdir ...
                '\Behavior\TrainingIncluded\AcrossDays_probe_FourNeuralMice_' exlnan_lab{exlnan}])
        ...
        % across trials probes
        if sum(ismember(figs_to_run,0))>0
            figure; hold on
            for itrialtype = 1:size(alltrial_AllProbes,3)
                dat = squeeze(mean(alltrial_AllProbes(1:10,:,1:4,itrialtype),2 ...
                    ,'omitnan'));
